...
c     icon_crust=3:
     ...
     2              nu_e_s_imp,nu_e_l_imp) 
               ...
               print *,'sub. con_crust: icon_crust incorrectly def !'
               stop
            end if
...
c     icon_crust=2:
     ...
     2              nu_e_s1,nu_e_l1) 
               call con_crust_e_phonon_BY(T,rho,A,A1,Z,
     1              sigma2_ph,lambda2_ph,debug,
     2              nu_e_s2_ph,nu_e_l2_ph,rhodrip) 
               call con_crust_e_imp_YU(T,rho,A,A1,Z,Q_imp,
     1              sigma2_imp,lambda2_imp,debug,
     2              nu_e_s2_imp,nu_e_l2_imp) 
               nu_e_s2=nu_e_s2_ph+nu_e_s2_imp
...
c     ***************************
     ...
     1     ' T, rho=',T,rho
      if (rho.gt.1.d10) then
         print *,'Subroutine con_env_e_phon_ion: rho > 1e10 !'
         stop
      end if
c     ***************************
      n_i=rho/A*6.022d23
...
c     Get the type and size of nuclei using Gnedin et al subroutine:
      ...
      if (ifs.eq.0) then
         ...
         print *,'Sub. ''con_e_phon_ion_GYP'': ifs badly defined'
         stop
      end if
c     ******
      n_i=rho/(A*mu)
...
c     *********************************************************************
     ...
     2     nu_s,nu_k)
      ...
      print *,'AWS: Removed because this required a data file.'
      stop
